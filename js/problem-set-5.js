...
function credit() {
  ...
  let odd_sum=0
  while (true) {
    if (((card.length==16 || card.length==13 || card.length==15) && Number.isInteger(card)))
     card = Number(card);
     break;
  }
...
document.getElementById("report-card-output").innerHTML= "Tests:" + testAverage + "</br>Quizzes:" + "</br>Homeworks:" + homeworkAverage;
